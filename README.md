...
- **Accuracy**: Demonstrate that Base-5 dual experts + INT8 residual highways + spectral orthogonality recover mAP (mean Average Precision) within ±1.0 percentage point of the FP32 baseline on COCO val2017 (e.g., baseline 50.0% → acceptable range 49.0%–51.0%).
...
- Spectral separation score (1 - normalized cross-spectrum correlation; see spectral orthogonality loss in Section 5.5) remains >0.3 after convergence; this is an initial target to keep expert frequency content meaningfully distinct and should be tuned once empirical distributions are observed.
